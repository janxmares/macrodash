...
lbs_grey <- tribble(~vek_txt, ~day_sameyear_floored, ~min, ~txt,
               factor("Total"), as.Date("1970-05-01"), 12.1, "range in \n2011-19",
               )
lbs_blue <- tribble(~vek_txt, ~day_sameyear_floored, ~min, ~txt,
               factor("Total"), as.Date("1970-02-15"), 11.5, "2020"
               )
```
...
ggplot(data = zmr_bands, mapping = aes(group = vek_txt)) +
  ...
  labs(x = NULL, y = NULL,
       ...
       subtitle = "Deaths per 100,000 by week, 2011-2020. Blue = 2020, grey = range in 2011-2019",
       caption = str_glue("Based on CZSO dataset 130185 (deaths) a 130142 (age) | last checked {today_fmt} | Last data for {last_date_fmt}.")) +
